...
Run exactly as in `oferta` mode (read `modes/oferta.md` for all A-F blocks).
...
1. **Extract form questions**: Use Playwright to navigate to the form and take a snapshot. If questions cannot be extracted, use the generic questions below.
2. **Generate responses** following the tone guidelines (see below).
...
**Language**: Always match the language of the JD (EN default). Apply `/tech-translate`.

## Step 5 — Update Tracker
Log in `data/applications.md` with all columns, including Report and PDF marked as ✅.
