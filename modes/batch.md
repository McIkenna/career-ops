# Mode: batch — Bulk Job Offer Processing
...
1. **Read state**: `batch/batch-state.tsv` → determine what has already been processed
...
Options:
- `--dry-run` — lists pending items without executing
- `--retry-failed` — only retries failed offers
...
- If it crashes → re-run → reads `batch-state.tsv` → skips completed items
