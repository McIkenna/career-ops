...
Interactive mode for when the candidate is filling out an application form in Chrome. Reads what's on screen, loads previous job offer context, and generates personalized responses for each form question.
...
- **If adapting**: Adjust the responses to the new role without re-evaluating
